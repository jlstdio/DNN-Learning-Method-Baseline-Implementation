"""
PhyMask 방식으로 HuggingFace 모델을 PAMAP2 및 HHAR 데이터셋에 학습.
물리적 인식 기반 적응형 마스킹으로 중요한 구간을 우선 마스킹하여 학습.
...
from phyMask import PhyMask, train_phymask
from prepare import load_pamap2, load_hhar
from hf_backbones import load_model, model_id_to_name


def main():
    parser = argparse.ArgumentParser(description='PhyMask Training with HuggingFace Models')
    ...
    parser.add_argument('--mask_ratio', type=float, default=0.5,
                        help='Fraction of timesteps to mask (default: 0.5)')
    ...
    model = PhyMask(encoder, input_dim=input_dim, d_model=d_model).to(device)
    ...
    save_prefix = f'phymask_{model_name}_{args.dataset}'

    # ── 학습 루프 ──
    print(f"\n{'='*60}")
    print(f"PhyMask Training: {args.model_id}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for (batch_x,) in pbar:
            ...
            loss = train_phymask(model, optimizer, batch_x,
